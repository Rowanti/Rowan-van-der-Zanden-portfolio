...
byte echoPins[] = {6, 8, 10}; // first 3 are facing the front
byte trigPins[] = {7, 9, 11};

byte echoSide[] = {4, 12};// facing the sides
byte trigSide[] = {5, 13};
...
void receiveEvent(int howMany){
  while(1<Wire.available()){
    byte steer = Wire.read();
  }
  byte speeds = Wire.read();
}
...
void loop() {
 ...
 Serial.println("_____________________________");
  bool sideThing = false;
  int speedModifier = 1;
  for (byte i=0;i<m;i++){
    Serial.println(measure(trigSide[i], echoSide[i]));
    if(measure(trigSide[i], echoSide[i])<40){
      sideThing = true;
      Serial.println("halfed");
      speedModifier = 2;
    }
  }
  if (!sideThing){
  ...
  if (object){
    ...
    if (timer - lastStop > 2000){
      Serial.println(speedModifier);
      Send(0, (80/speedModifier));
